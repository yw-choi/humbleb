...
### Backend Gate
- [ ] FastAPI 서버 기동, `/health` 200 응답
- [ ] DB 마이그레이션 실행 (Member, Schedule, Attendance 테이블 생성)
- [ ] 멤버 시드 데이터 30명 삽입 확인
- [ ] 카카오 OAuth 플로우 동작 (로그인 → JWT 쿠키 → /api/members/me 본인 반환)
...
- [ ] 참가/불참 버튼 동작 (full 기본, late/early 선택 가능)
...
### Frontend Gate
- [ ] 운영진 페이지 접근 — 일반 멤버는 접근 불가 (리다이렉트 또는 404)
- [ ] 일정 CRUD UI 동작
- [ ] 게스트 등록 폼 (이름, 성별, 실력 선택)
- [ ] 카톡 공유 텍스트 복사 버튼 — 클립보드 복사 + 토스트
